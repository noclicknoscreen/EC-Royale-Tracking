...
//  Handle[] handles = new Handle[4];   // handles declaration
  ...
  DBox(int id, float dsize) {
    ...
    handles[id*4 + 0] = new Handle(getDBoxDataCoor(id, 0).x, getDBoxDataCoor(id, 0).y, 0, 0, hsize, handles);
    handles[id*4 + 1] = new Handle(getDBoxDataCoor(id, 1).x, getDBoxDataCoor(id, 1).y, 0, 0, hsize, handles);
    handles[id*4 + 2] = new Handle(getDBoxDataCoor(id, 2).x, getDBoxDataCoor(id, 2).y, 0, 0, hsize, handles);
    handles[id*4 + 3] = new Handle(getDBoxDataCoor(id, 3).x, getDBoxDataCoor(id, 3).y, 0, 0, hsize, handles);

    // define vertices for polygon detection
    for (int i = 0; i < 4; i++) {
      vertices[i] = new Vect2(handles[id*4 + i].getX(), handles[id*4 + i].getY());
    }
  }
  ...
  float mouseDistance() {
    Vect2 linePoint1 = new Vect2(handles[id*4].getX(), handles[id*4].getY());
    Vect2 linePoint2 = new Vect2(handles[id*4+1].getX(), handles[id*4+1].getY());
    ...
    if (Space2.insidePolygon(mouseVect, vertices)) {
      ...
      distance = map(distance, -20, 200, 1, 0);
    } else {
      return 0;
    }
    return distance;
  ...
  void display() {
    ...
    beginShape();
    for (int i = 0; i < 4; i++) {
      vertex(handles[id*4 + i].getX(), handles[id*4 + i].getY());
      vertices [i] = new Vect2(handles[id*4 + i].getX(), handles[id*4 + i].getY());
    }
    endShape(CLOSE);
    ...
    strokeWeight(2);
    beginShape();
    vertex(handles[id*4].getX(), handles[0].getY());
    vertex(handles[id*4 + 1].getX(), handles[1].getY());
    endShape();

    // display handles
    for (int i = 0; i < 4; i++) {
      handles[id*4+i].display(i);
    }
    ...
    text("ID "+ id + " / population: " + population + " / distance: " + distance, 30, height-80 +25*id);
  }
